...
const STORAGE_KEYS = {
  ...
  DECISION_HELPER: 'decoration_decision_helper_v1',
  CUSTOM_STAGES: 'reno_custom_stages',     // custom stage data
  // ===== 排期日历模块 =====
  SCHEDULE_TASKS:      'reno_schedule_tasks',      // [{ id, name, startDate, estimatedWorkDays, endDate, workerId, workerName, area, content, isNoisy, needsAcceptance, status, color, note, createdAt, updatedAt }]
  NO_WORK_DAYS:        'reno_no_work_days',         // [{ id, date, reason, type, allowQuiet, note, createdAt, updatedAt }]
  DAILY_LOGS:          'reno_daily_logs',           // [{ id, date, workContent, workers, materialStatus, issuesText, tomorrowPlan, note, createdAt, updatedAt }]
  MATERIAL_REMINDERS:  'reno_material_reminders',   // [{ id, name, relatedTaskId, remindDate, expectedArrivalDate, actualArrivalDate, status, note, createdAt, updatedAt }]
  ACCEPTANCE_NODES:    'reno_acceptance_nodes',     // [{ id, name, relatedTaskId, date, checklist, result, allowNextStep, note, createdAt, updatedAt }]
  ISSUE_RECORDS:       'reno_issue_records',        // [{ id, date, description, responsiblePerson, deadline, status, note, createdAt, updatedAt }]
  SCHEDULE_SETTINGS:   'reno_schedule_settings',    // { satNoWork, sunNoWork, weekendAllowQuiet, autoMaterial, autoAcceptance }
};
...
const STORAGE_VALUE_TYPES = {
  ...
  NO_WORK_DAYS:       'array',
  DAILY_LOGS:         'array',
  MATERIAL_REMINDERS: 'array',
  ACCEPTANCE_NODES:   'array',
  ISSUE_RECORDS:      'array',
