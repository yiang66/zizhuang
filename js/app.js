...
(function () {
  ...
  function renderDashboardBudgetChart() {
    ...
    const catColors = [
      '#f97316','#3b82f6','#8b5cf6','#14b8a6','#f59e0b','#ec4899','#22c55e'
    ];
  ...
  function renderDashboardRecentExpenses() {
    ...
    const items = StorageUtil.get(STORAGE_KEYS.BUDGET_V2, []);
    const BV2_CAT_NAMES = {
      demolition: '①拆改/设计', hardwork: '②硬装施工', doors: '③门窗设备',
      custom: '④定制柜', sanitary: '⑤厨卫五金', lighting: '⑥灯具家电', soft: '⑦软装家具',
    };
  ...
  function renderWorkflow() {
    ...
    container.innerHTML = stages.map((stage, stageIndex) => {
      ...
      return `
        <div class="stage-card status-${status} ${isExpanded ? 'expanded' : ''}" id="stage-${stage.id}" data-stage="${stage.id}"
  ...
  const BV2_CATS = [
    { id: 'demolition', name: '①前期拆改/设计监理', color: '#f97316' },
    { id: 'hardwork',   name: '②水电/瓦/木/油硬装', color: '#3b82f6' },
    { id: 'doors',      name: '③门窗暖通设备',       color: '#8b5cf6' },
    { id: 'custom',     name: '④全屋定制柜',          color: '#14b8a6' },
    { id: 'sanitary',   name: '⑤厨卫洁具五金',        color: '#f59e0b' },
    { id: 'lighting',   name: '⑥灯具家电',            color: '#ec4899' },
    { id: 'soft',       name: '⑦软装家具',            color: '#22c55e' },
  ];
  ...
  function renderBudgetSpreadsheet() {
    ...
    const PRIORITY_LABEL = { must: '🔴刚需', optional: '🟡可减配', nice: '🟢锦上添花' };
    const catMap = {};
    ...
    BV2_CATS.forEach(cat => {
      ...
      html += `
        <tr class="bv2-subtotal">
          <td></td>
          <td class="subtotal-label" colspan="8">${cat.name} 小计</td>
    ...
    tfoot.innerHTML = `
      <tr class="bv2-grand-total">
        <td colspan="9" class="grand-total-label">📊 全部合计</td>
  ...
  window.onXlNumChange = function (input) {
    ...
    if (!item) return;
    item[field] = value;
    // Auto-calc budget from qty × unit price
  ...
  window.addBv2Row = function (catId) {
    ...
    setTimeout(() => {
      const editables = document.querySelectorAll('.xl-editable[data-field="name"]');
  ...
  function renderWorkerTypeList() {
    ...
    let html = allWorkerTypes.map(wt => {
  ...
  window.deleteWorker = function (workerId) {
    if (!confirm('确定要删除这个工人的信息吗？相关沟通记录也会一起删除。')) return;
    let workers = StorageUtil.get(STORAGE_KEYS.WORKERS, []);
    workers = workers.filter(w => w.id !== workerId);
    StorageUtil.set(STORAGE_KEYS.WORKERS, workers);

    // Also delete related call records
    let calls = StorageUtil.get(STORAGE_KEYS.CALL_RECORDS, []);
    calls = calls.filter(c => c.workerId !== workerId);
